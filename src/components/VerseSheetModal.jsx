...
export default function VerseSheetModal({ sheetVerse, setSheetVerse, selectedBook, currentChapter, translation, setTab }) {
  ...
  return (
    <>
      <div className="ov" style={{ display: 'block' }} onClick={() => setSheetVerse(null)}></div>
      <div className="vs open" style={{ display: 'block', transform: 'translateX(-50%) translateY(0)' }}>
        ...
        <div className="hlrow">
          ...
          <div className={`hlsw ${curHl === 'y' ? 'pk' : ''}`} style={{ background: 'rgba(255,215,0,.55)' }} onClick={() => toggleHighlight(vKey, 'y')}></div>
          <div className={`hlsw ${curHl === 'r' ? 'pk' : ''}`} style={{ background: 'rgba(255,80,80,.55)' }} onClick={() => toggleHighlight(vKey, 'r')}></div>
          <div className={`hlsw ${curHl === 'g' ? 'pk' : ''}`} style={{ background: 'rgba(50,200,80,.55)' }} onClick={() => toggleHighlight(vKey, 'g')}></div>
          <div className={`hlsw ${curHl === 'b' ? 'pk' : ''}`} style={{ background: 'rgba(40,140,255,.55)' }} onClick={() => toggleHighlight(vKey, 'b')}></div>
          <div className="hlclr" onClick={() => toggleHighlight(vKey, null)}>✕</div>
        </div>
        <div className="shacts">
          <button className="sact" onClick={() => { addBookmark({ id: Date.now(), bk: selectedBook.ab, bn: selectedBook.n, ch: currentChapter, vn: sheetVerse.verse, text: sheetVerse.text.slice(0, 120), date: new Date().toLocaleDateString() }); setSheetVerse(null); }}>
          ...
          <button className="sact" onClick={() => { navigator.clipboard.writeText(`"${sheetVerse.text}" — ${selectedBook.n} ${currentChapter}:${sheetVerse.verse}`); setSheetVerse(null); }}>
