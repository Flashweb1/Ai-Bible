...
if (openrouterKey) {
  providers.push({
    ...
    defaultHeaders: {
      'HTTP-Referer': process.env.SITE_URL || 'https://scripturai.netlify.app',
      'X-Title': 'Scripturai',
    },
    models: [
...
console.log('AI Providers enabled:', providerClients.map(p => p.name).join(', ') || 'None');
...
app.post('/api/ai/ask', aiLimiter, async (req, res) => {
  ...
  if (!providerClients || providerClients.length === 0) {
    return res.status(503).json({ error: "AI Service unconfigured on server. Check API keys." });
  }
...
app.post('/api/ai/ask/stream', aiLimiter, async (req, res) => {
  ...
  if (!providerClients || providerClients.length === 0) {
    res.status(503).json({ error: "AI Service unconfigured on server. Check API keys." });
    return;
  }
